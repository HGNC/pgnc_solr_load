...
class TestDataLoadIntegration:
    ...
    def test_complete_data_loading_workflow(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False) as temp_file:
            ...
            writer.writerow([
                'primary_id', 'gene_symbol_string', 'gene_name_string',
                'alias_symbol_string', 'alias_name_string',
                'location_string', 'locus_type_string',
                'external_id_ensembl', 'external_id_refseq', 'external_id_ucsc'
            ])
            writer.writerow([
                'TEST.1', 'TEST_SYMBOL', 'Test Gene Name',
                'TEST_ALIAS', 'Test Alias Name',
                '1q21.1', 'gene with protein product',
                'ENSG123456', 'NM_123456', 'uc001abc.1'
            ])
        ...
        try:
            ...
            with patch('main.sa.create_engine') as mock_create_engine:
                ...
                with patch('main.sa.orm.sessionmaker') as mock_sessionmaker:
                    ...
                    with patch.object(GeneDataLoader, '_get_gene_and_creator', return_value=(mock_gene, mock_user)):
                        with patch.object(GeneDataLoader, '_process_symbols'):
                            with patch.object(GeneDataLoader, '_process_names'):
                                with patch.object(GeneDataLoader, '_process_location'):
                                    with patch.object(GeneDataLoader, '_process_locus_type'):
                                        with patch.object(GeneDataLoader, '_process_crossrefs'):
                                            ...
                                            assert loader.df.iloc[0]['primary_id'] == 'TEST.1'
    ...
    def test_error_handling_during_workflow(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False) as temp_file:
            ...
            writer.writerow(['ERROR.1', 'ERROR_SYMBOL'])
            temp_csv_path = temp_file.name
        ...
        try:
            ...
            with patch('main.sa.create_engine') as mock_create_engine:
                ...
                with patch('main.sa.orm.sessionmaker') as mock_sessionmaker:
                    ...
                    with patch.object(GeneDataLoader, '_get_gene_and_creator', side_effect=Exception("Database error")):
                        with patch('builtins.print') as mock_print:
                            ...
                            error_found = any("Error processing row 0: Database error" in call for call in error_calls)
                            assert error_found, f"Expected error message not found in calls: {error_calls}"
    ...
    def test_csv_validation_edge_cases(self):
        ...
        with patch('builtins.print') as mock_print:
            ...
            mock_print.assert_called_with("Error: File not found at path: /non/existent/file.csv")
    ...
    def test_error_propagation_patterns(self):
        ...
        with patch.object(loader, '_process_approved_symbol', side_effect=ValueError("Symbol exists")):
            with patch.object(loader, '_process_alias_symbols'):
                with patch('builtins.print') as mock_print:
                    ...
                    error_calls = [str(call) for call in mock_print.call_args_list]
                    error_found = any("already has approved symbol TEST_SYMBOL. Skipping" in call for call in error_calls)
                    assert error_found, f"Expected error message not found in calls: {error_calls}"
